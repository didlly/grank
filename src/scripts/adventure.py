...
def adventure(Client) -> None:
    ...
    if "description" in latest_message["embeds"][0].keys():
        if "You reached the end of your adventure!" in latest_message["embeds"][0]["description"]:
            ...
            backpack = ", ".join(backpack[index].lower() for index in range(1, len(backpack), 2))
            ...
            Client.log("DEBUG", f"Ended adventure: `{adventure}`; Items taken: `{backpack}`; Items found: `{found}`; Coins found: `{coins}`; Amount of Interactions: `{interactions}`.")
        ...
        if "Choose an Adventure" in latest_message["embeds"][0]["author"]["name"]:
            ...
            if "footer" in latest_message["embeds"][0].keys():
                if Client.Repository.config["logging"]["debug"]:
                    Client.log("DEBUG", "User does not have item `adventure ticket`. Buying adventure ticket now.")
    ...
    if "You ran out of fuel! What next?" in latest_message["embeds"][0]["description"]:
        Client.log("DEBUG", "Fuel loss adventure phase. Choosing `Search a planet` option.")
        
        custom_id = latest_message["components"][0]["components"][0]["custom_id"]
    elif "You accidentally bumped into the Webb Telescope." in latest_message["embeds"][0]["description"]:
        Client.log("DEBUG", "Webb telescope adventure phase. Choosing `Try and fix it` option.")
        
        custom_id = latest_message["components"][0]["components"][0]["custom_id"]
    elif "A friendly alien approached you slowly." in latest_message["embeds"][0]["description"]:
    ...
    elif "You got abducted by a group of aliens," in latest_message["embeds"][0]["description"]:
        Client.log("DEBUG", "Alien abduction adventure. Choosing `Flee` option.")
        
        custom_id = latest_message["components"][0]["components"][-1]["custom_id"]
    else:
        Client.log("WARNING", "Unknown `pls adventure` phase. Clicking a random button.")
