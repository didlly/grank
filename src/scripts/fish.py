...
def fish(username, channel_id, token, config, user_id, cwd):
    send_message(channel_id, token, config, username, "pls fish")

    latest_message = retreive_message(channel_id, token, config, username, "pls fish", user_id)

    if not latest_message[0]:
        return

    if latest_message[-1]["content"] == "You don't have a fishing pole, you need to go buy one. You're not good enough to catch them with your hands.":
        if config["logging"]["debug"]:
            log(username, "DEBUG", "User does not have item `fishing pole`. Buying fishing pole now.")
        
        if config["commands"]["auto_buy"]:
            from scripts.buy import buy
            buy(username, channel_id, token, config, log, user_id, cwd, "fishing pole")
            return
...
def fish_parent(username, channel_id, token, config, user_id, cwd):
    while True:
        ...
        try:
            fish(username, channel_id, token, config, user_id, cwd)
        except Exception:
