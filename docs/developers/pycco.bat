...
:: Add `- pycco` on the the names of all files generated by pycco
for %%a in (*.html) do ren "%%~a" "%%~na -pycco%%~xa"

:: Replace pycco's main.py with the following code to fix glob patterns not working (on Windows at least) and other bugs
    ...
    for line in lines:
        ...
        if (
            ...
            if re.match(r" {{{:d}}}".format(len(indent_level)), line):
                docs_text += line[len(indent_level) :].lstrip() + "\n"
            else:
